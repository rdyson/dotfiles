...
function activate(context) {
    ...
    vscode.workspace.onDidSaveTextDocument((_document) => {
        const onFormat = vscode.workspace.getConfiguration('formatOnSave')['on'];
        if (onFormat !== true) {
            return;
        }
        const editor = vscode.window.activeTextEditor;
...
let fileCoverage = {};
function runOPAFormatter(context, content, editor, reject, resolve, selectionRange) {
    opa.runWithStatus(context, 'opa', ['fmt'], content, (code, stderr, stdout) => {
        if (!editor) {
            return [];
        }
        if (code !== 0) {
...
class RegoDefinitionProvider {
    provideDefinition(document, position, _token) {
        const args = ['oracle', 'find-definition', '--stdin-buffer'];
        ifInWorkspace(() => {
        ...
        return new Promise((resolve, reject) => {
            ...
            opa.runWithStatus(undefined, 'opa', args, document.getText(), (code, stderr, stdout) => {
                if (code === 0) {
