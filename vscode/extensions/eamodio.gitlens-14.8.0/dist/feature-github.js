...
`;let GitHubApi=class GitHubApi{_onDidReauthenticate=new A.EventEmitter;get onDidReauthenticate(){return this._onDidReauthenticate.event}_disposable;constructor(e){this._disposable=_.i.onDidChangeAny(e=>{(_.i.changedCore(e,["http.proxy","http.proxyStrictSSL"])||_.i.changed(e,["outputLevel","proxy"]))&&this.resetCaches()})}dispose(){this._disposable.dispose()}resetCaches(){this._proxyAgent=null,this._defaults.clear(),this._enterpriseVersions.clear()}_proxyAgent=null;get proxyAgent(){if(!H.e6)return null===this._proxyAgent&&(this._proxyAgent=(0,U.aE)()),this._proxyAgent}async getAccountForCommit(e,t,r,i,o,n){let a=(0,M.b1)();try{let s=`query getAccountForCommit(
	$owner: String!
	$repo: String!
	$ref: GitObjectID!
	$avatarSize: Int
...
}`,s=await this.graphql(e,t,a,{...o,owner:r,repo:i},n),l=s?.repository?.defaultBranchRef?.name??void 0;if(null==l)return;return{provider:e,name:l}}catch(t){if(t instanceof F.SY)return;throw this.handleException(t,e,n)}}async getIssueOrPullRequest(e,t,r,i,o,n){let a=(0,M.b1)();try{let s=`query getIssueOrPullRequest(
	$owner: String!
	$repo: String!
	$number: Int!
) {
...
}`,l=await this.graphql(e,t,s,{...n,owner:r,repo:i,number:o},a),u=l?.repository?.issueOrPullRequest;if(null==u)return;return{provider:e,type:u.type,id:String(o),nodeId:u.id,date:new Date(u.createdAt),title:u.title,closed:u.closed,closedDate:null==u.closedAt?void 0:new Date(u.closedAt),url:u.url,state:(0,L.fromGitHubPullRequestState)(u.state)}}catch(t){if(t instanceof F.SY)return;throw this.handleException(t,e,a)}}async getPullRequestForBranch(e,t,r,i,o,n){let a=(0,M.b1)();try{let s=`query getPullRequestForBranch(
	$owner: String!
	$repo: String!
	$branch: String!
	$limit: Int!
...
}`,s=await this.graphql(void 0,e,a,{owner:t,repo:r,ref:i,path:o},n);if(null==s)return W;let l=s.repository?.object?.blame?.ranges;if(null==l||0===l.length)return{ranges:[],viewer:s.viewer?.name};return{ranges:l,viewer:s.viewer?.name}}catch(e){if(e instanceof F.SY)return W;throw this.handleException(e,void 0,n)}}async getBranches(e,t,r,i){let o=(0,M.b1)();try{let n=`query getBranches(
	$owner: String!
	$repo: String!
	$branchQuery: String
	$cursor: String
...
}`,a=await this.graphql(void 0,e,n,{owner:t,repo:r,branchQuery:i?.query,cursor:i?.cursor,limit:Math.min(100,i?.limit??100)},o);if(null==a)return Q;let s=a.repository?.refs;if(null==s)return Q;return{paging:{cursor:s.pageInfo.endCursor,more:s.pageInfo.hasNextPage},values:s.nodes}}catch(e){if(e instanceof F.SY)return Q;throw this.handleException(e,void 0,o)}}async getCommit(e,t,r,i){let o=(0,M.b1)();try{let n=await this.request(void 0,e,"GET /repos/{owner}/{repo}/commits/{ref}",{owner:t,repo:r,ref:i},o),a=n?.data;if(null==a)return;let{commit:s}=a;return{oid:a.sha,parents:{nodes:a.parents.map(e=>({oid:e.sha}))},message:s.message,additions:a.stats?.additions,changedFiles:a.files?.length,deletions:a.stats?.deletions,author:{avatarUrl:a.author?.avatar_url??void 0,date:s.author?.date??new Date().toString(),email:s.author?.email??void 0,name:s.author?.name??""},committer:{date:s.committer?.date??new Date().toString(),email:s.committer?.email??void 0,name:s.committer?.name??""},files:a.files}}catch(e){if(e instanceof F.SY)return;throw this.handleException(e,void 0,o)}}async getCommitForFile(e,t,r,i,o){if((0,k.kp)(i))return this.getCommit(e,t,r,i);let n=await this.getCommits(e,t,r,i,{limit:1,path:o});if(0!==n.values.length)return{...await this.getCommit(e,t,r,n.values[0].oid)??n.values[0],viewer:n.viewer}}async getCommitBranches(e,t,r,i,o){let n=(0,M.b1)();try{let a=`query getCommitBranches(
	$owner: String!
	$repo: String!
	$since: GitTimestamp!
	$until: GitTimestamp!
...
}`,s=await this.graphql(void 0,e,a,{owner:t,repo:r,since:o.toISOString(),until:o.toISOString()},n),l=s?.repository?.refs?.nodes;if(null==l)return[];let u=[];for(let e of l)for(let t of e.target.history.nodes)if(t.oid===i){u.push(e.name);break}return u}catch(e){if(e instanceof F.SY)return[];throw this.handleException(e,void 0,n)}}async getCommitCount(e,t,r,i){let o=(0,M.b1)();try{let n=`query getCommitCount(
	$owner: String!
	$repo: String!
	$ref: String!
) {
...
}`,a=await this.graphql(void 0,e,n,{owner:t,repo:r,ref:i},o);return a?.repository?.ref?.target.history.totalCount}catch(e){if(e instanceof F.SY)return;throw this.handleException(e,void 0,o)}}async getCommitOnBranch(e,t,r,i,o,n){let a=(0,M.b1)();try{let s=`query getCommitOnBranch(
	$owner: String!
	$repo: String!
	$ref: String!
	$since: GitTimestamp!
...
}`,l=await this.graphql(void 0,e,s,{owner:t,repo:r,ref:`refs/heads/${i}`,since:n.toISOString(),until:n.toISOString()},a),u=l?.repository?.ref.target.history.nodes;if(null==u)return[];let c=[];for(let e of u)if(e.oid===o){c.push(i);break}return c}catch(e){if(e instanceof F.SY)return[];throw this.handleException(e,void 0,a)}}async getCommits(e,t,r,i,o){let n=(0,M.b1)();if(o?.limit===1&&o?.path==null)return this.getCommitsCoreSingle(e,t,r,i);try{let a;let s=`query getCommits(
	$owner: String!
	$repo: String!
	$ref: String!
	$path: String
...
}`,a=await this.graphql(void 0,e,n,{owner:t,repo:r,ref:i},o);if(null==a)return Q;let s=a.repository?.object;return null!=s?{values:[s],viewer:a.viewer.name}:Q}catch(e){if(e instanceof F.SY)return Q;throw this.handleException(e,void 0,o)}}async getCommitRefs(e,t,r,i,o){let n=(0,M.b1)();try{let a=`query getCommitRefs(
	$owner: String!
	$repo: String!
	$ref: String!
	$after: String
...
}`,s=await this.graphql(void 0,e,a,{owner:t,repo:r,ref:i,path:o?.path,first:o?.first,last:o?.last,after:o?.after,before:o?.before,since:o?.since,until:o?.until},n),l=s?.repository?.object?.history;if(null==l)return;return{pageInfo:l.pageInfo,totalCount:l.totalCount,values:l.nodes}}catch(e){if(e instanceof F.SY)return;throw this.handleException(e,void 0,n)}}async getCommitTags(e,t,r,i,o){let n=(0,M.b1)();try{let a=`query getCommitTags(
	$owner: String!
	$repo: String!
	$since: GitTimestamp!
	$until: GitTimestamp!
...
}`,s=await this.graphql(void 0,e,a,{owner:t,repo:r,since:o.toISOString(),until:o.toISOString()},n),l=s?.repository?.refs?.nodes;if(null==l)return[];let u=[];for(let e of l)for(let t of e.target.history.nodes)if(t.oid===i){u.push(e.name);break}return u}catch(e){if(e instanceof F.SY)return[];throw this.handleException(e,void 0,n)}}async getNextCommitRefs(e,t,r,i,o,n){let a;let s=await this.getCommitDate(e,t,r,n);if(null==s)return[];let l=await this.getCommitRefs(e,t,r,i,{path:o,first:1,since:s});if(null==l)return[];let u=`${l.pageInfo.startCursor.split(" ",1)[0]} ${l.totalCount}`;if([,a]=u.split(" ",2),a=Math.min(parseInt(a,10),5),null==(l=await this.getCommitRefs(e,t,r,i,{path:o,last:a,before:u})))return[];let c=[];for(let{oid:e}of l.values){if(e===n)break;c.push(e)}return c.reverse()}async getCommitDate(e,t,r,i){let o=(0,M.b1)();try{let n=`query getCommitDate(
	$owner: String!
	$repo: String!
	$sha: GitObjectID!
) {
...
}`,a=await this.graphql(void 0,e,n,{owner:t,repo:r,sha:i},o);return a?.repository?.object?.committer.date}catch(e){if(e instanceof F.SY)return;throw this.handleException(e,void 0,o)}}async getContributors(e,t,r){let i=(0,M.b1)();try{let o=await this.request(void 0,e,"GET /repos/{owner}/{repo}/contributors",{owner:t,repo:r,per_page:100},i),n=o?.data;if(null==n)return[];return o.data}catch(e){if(e instanceof F.SY)return[];throw this.handleException(e,void 0,i)}}async getDefaultBranchName(e,t,r){let i=(0,M.b1)();try{let o=`query getDefaultBranch(
	$owner: String!
	$repo: String!
) {
	repository(owner: $owner, name: $repo) {
...
}`,n=await this.graphql(void 0,e,o,{owner:t,repo:r},i);if(null==n)return;return n.repository?.defaultBranchRef?.name??void 0}catch(e){if(e instanceof F.SY)return;throw this.handleException(e,void 0,i)}}async getCurrentUser(e,t,r){let i=(0,M.b1)();try{let o=`query getCurrentUser(
	$owner: String!
	$repo: String!
) {
	viewer { name, email, login, id }
	repository(owner: $owner, name: $repo) { viewerPermission }
}`,n=await this.graphql(void 0,e,o,{owner:t,repo:r},i);if(null==n)return;return{name:n.viewer?.name,email:n.viewer?.email,username:n.viewer?.login,id:n.viewer?.id}}catch(e){if(e instanceof F.SY)return;throw this.handleException(e,void 0,i)}}async getRepositoryVisibility(e,t,r){let i=(0,M.b1)();try{let o=`query getRepositoryVisibility(
	$owner: String!
	$repo: String!
) {
	repository(owner: $owner, name: $repo) {
...
}`,n=await this.graphql(void 0,e,o,{owner:t,repo:r},i);if(n?.repository?.visibility==null)return;return"PUBLIC"===n.repository.visibility?"public":"private"}catch(e){if(e instanceof F.SY)return;throw this.handleException(e,void 0,i)}}async getTags(e,t,r,i){let o=(0,M.b1)();try{let n=`query getTags(
	$owner: String!
	$repo: String!
	$tagQuery: String
	$cursor: String
...
}`,a=await this.graphql(void 0,e,n,{owner:t,repo:r,tagQuery:i?.query,cursor:i?.cursor,limit:Math.min(100,i?.limit??100)},o);if(null==a)return Q;let s=a.repository?.refs;if(null==s)return Q;return{paging:{cursor:s.pageInfo.endCursor,more:s.pageInfo.hasNextPage},values:s.nodes}}catch(e){if(e instanceof F.SY)return Q;throw this.handleException(e,void 0,o)}}async resolveReference(e,t,r,i,o){let n=(0,M.b1)();try{if(!o){let o=`query resolveReference(
	$owner: String!
	$repo: String!
	$ref: String!
) {
...
}`,s=await this.graphql(void 0,e,a,{owner:t,repo:r,ref:i,path:o},n);return s?.repository?.object?.history.nodes?.[0]?.oid??void 0}catch(e){if(e instanceof F.SY)return;throw this.handleException(e,void 0,n)}}async searchCommits(e,t,r){let i,o,n;let a=(0,M.b1)(),s=Math.min(100,r?.limit??100);r?.cursor!=null?([i,o,n]=r.cursor.split(" ",3),i=parseInt(i,10),o=parseInt(o,10),n=parseInt(n,10)):(i=1,o=s,n=0);try{let s=await this.request(void 0,e,"GET /search/commits",{q:t,sort:r?.sort,order:r?.order,per_page:o,page:i},a),l=s?.data;if(null==l||0===l.items.length)return;let u=l.items.map(e=>({oid:e.sha,parents:{nodes:e.parents.map(e=>({oid:e.sha}))},message:e.commit.message,author:{avatarUrl:e.author?.avatar_url??void 0,date:e.commit.author?.date??e.commit.author?.date??new Date().toString(),email:e.author?.email??e.commit.author?.email??void 0,name:e.author?.name??e.commit.author?.name??""},committer:{date:e.commit.committer?.date??e.committer?.date??new Date().toString(),email:e.committer?.email??e.commit.committer?.email??void 0,name:e.committer?.name??e.commit.committer?.name??""}})),c=n+l.items.length,h=l.incomplete_results||l.total_count>c;return{pageInfo:{startCursor:`${i} ${o} ${n}`,endCursor:h?`${i+1} ${o} ${c}`:void 0,hasPreviousPage:l.total_count>0&&i>1,hasNextPage:h},totalCount:l.total_count,values:u}}catch(e){if(e instanceof F.SY)return;throw this.handleException(e,void 0,a)}}async searchCommitShas(e,t,r){let i,o,n;let a=(0,M.b1)(),s=Math.min(100,r?.limit??100);r?.cursor!=null?([i,o,n]=r.cursor.split(" ",3),i=parseInt(i,10),o=parseInt(o,10),n=parseInt(n,10)):(i=1,o=s,n=0);try{let s=await this.request(void 0,e,"GET /search/commits",{q:t,sort:r?.sort,order:r?.order,per_page:o,page:i},a),l=s?.data;if(null==l||0===l.items.length)return;let u=n+l.items.length,c=l.incomplete_results||l.total_count>u;return{pageInfo:{startCursor:`${i} ${o} ${n}`,endCursor:c?`${i+1} ${o} ${u}`:void 0,hasPreviousPage:l.total_count>0&&i>1,hasNextPage:c},totalCount:l.total_count,values:l.items.map(e=>({sha:e.sha,authorDate:new Date(e.commit.author.date).getTime(),committerDate:new Date(e.commit.committer?.date??e.commit.author.date).getTime()}))}}catch(e){if(e instanceof F.SY)return;throw this.handleException(e,void 0,a)}}_enterpriseVersions=new Map;async getEnterpriseVersion(e,t,r){let i=this._enterpriseVersions.get(t);if(null!=i)return i;if(null===i)return;let o=(0,M.b1)();try{let n=await this.request(e,t,"GET /meta",r,o),a=n?.data?.installed_version;i=a?(0,j.K0)(a):null}catch(e){i=null}return this._enterpriseVersions.set(t,i),i??void 0}async graphql(e,t,r,i,o,n){try{let o;if(null!=n){if(n.isCancellationRequested)throw new F.WO;o=new AbortController,n.onCancellationRequested(()=>o.abort()),i={...i,request:{...i?.request,signal:o.signal}}}return await (0,U.uE)(e?.getIgnoreSSLErrors()??!1,()=>this.getDefaults(t,D)(r,i))}catch(r){if(r instanceof G){switch(r.errors?.[0]?.type){case"NOT_FOUND":throw new F.SY(r);case"FORBIDDEN":throw new F.yA("github",F.ms.Forbidden,r);case"RATE_LIMITED":{let e;let i=r.headers?.["x-ratelimit-reset"];throw null!=i&&Number.isNaN(e=parseInt(i,10))&&(e=void 0),new F.sT(r,t,e)}}O.YJ.isDebugging&&A.window.showErrorMessage(`GitHub request failed: ${r.errors?.[0]?.message??r.message}`)}else r instanceof RequestError||"AbortError"===r.name?this.handleRequestError(e,t,r,o):O.YJ.isDebugging&&A.window.showErrorMessage(`GitHub request failed: ${r.message}`);throw r}}async request(e,t,r,i,o,n){try{let o;if(null!=n){if(n.isCancellationRequested)throw new F.WO;o=new AbortController,n.onCancellationRequested(()=>o.abort()),i={...i,request:{...i?.request,signal:o.signal}}}return await (0,U.uE)(e?.getIgnoreSSLErrors()??!1,()=>this.getDefaults(t,S)(r,i))}catch(r){throw r instanceof RequestError||"AbortError"===r.name?this.handleRequestError(e,t,r,o):O.YJ.isDebugging&&A.window.showErrorMessage(`GitHub request failed: ${r.message}`),r}}_defaults=new Map;getDefaults(e,t){let r=this._defaults.get(t);null==r&&(r=new Map,this._defaults.set(t,r));let i=r.get(e);return null==i&&(i=t.defaults({headers:{authorization:`token ${e}`},request:{agent:this.proxyAgent,fetch:H.e6?(e,t)=>{if(null!=t.headers){let{"user-agent":e,...r}=t.headers;e&&(t.headers=r)}return(0,U.I5)(e,t)}:U.I5,hook:"debug"===O.YJ.logLevel||O.YJ.isDebugging?async(e,t)=>{let r=(0,B.i)(`[GITHUB] ${t.method} ${t.url}`,{log:!1});try{return await e(t)}finally{let e;try{if("string"==typeof t.query){let r=/(^[^({\n]+)/.exec(t.query);e=` ${r?.[1].trim()??t.query}`}}catch{}r?.stop({message:e})}}:void 0}}),r.set(e,i)),i}handleRequestError(e,t,r,i){if("AbortError"===r.name)throw new F.WO(r);switch(r.status){case 404:case 410:case 422:throw new F.SY(r);case 401:throw new F.yA("github",F.ms.Unauthorized,r);case 403:if(r.message.includes("rate limit")){let e;let i=r.response?.headers?.["x-ratelimit-reset"];throw null!=i&&Number.isNaN(e=parseInt(i,10))&&(e=void 0),new F.sT(r,t,e)}throw new F.yA("github",F.ms.Forbidden,r);case 500:O.YJ.error(r,i),null!=r.response&&(e?.trackRequestException(),(0,T.uY)(`${e?.name??"GitHub"} failed to respond and might be experiencing issues.${null==e||"github"===e.id?" Please visit the [GitHub status page](https://githubstatus.com) for more information.":""}`));return;case 502:if(O.YJ.error(r,i),r.message.includes("timeout")){e?.trackRequestException(),(0,T.Oc)(e?.name??"GitHub");return}break;default:if(r.status>=400&&r.status<500)throw new F.OW(r)}O.YJ.error(r,i),O.YJ.isDebugging&&A.window.showErrorMessage(`GitHub request failed: ${r.response?.errors?.[0]?.message??r.message}`)}handleException(e,t,r){return O.YJ.error(e,r),e instanceof F.yA&&this.showAuthenticationErrorMessage(e,t),e}async showAuthenticationErrorMessage(e,t){if(e.reason===F.ms.Unauthorized||e.reason===F.ms.Forbidden){let r="Reauthenticate";await A.window.showErrorMessage(`${e.message}. Would you like to try reauthenticating${e.reason===F.ms.Forbidden?" to provide additional access":""}?`,r)===r&&(await t?.reauthenticate(),this.resetCaches(),this._onDidReauthenticate.fire())}else A.window.showErrorMessage(e.message)}async createEnterpriseAvatarUrl(e,t,r,i,o){o=o??16;let n=await this.getEnterpriseVersion(e,t,{baseUrl:r});if((0,j.op)(n,">= 3.0.0")){let n;let a=(0,Y.S)(i);null!=a&&A.Uri.parse(r).authority===a.authority&&(null!=a.userId?n=`${r}/enterprise/avatars/u/${encodeURIComponent(a.userId)}?s=${o}`:null!=a.login&&(n=`${r}/enterprise/avatars/${encodeURIComponent(a.login)}?s=${o}`)),null==n&&(n=`${r}/enterprise/avatars/u/e?email=${encodeURIComponent(i)}&s=${o}`);let s=await (0,U.uE)(e?.getIgnoreSSLErrors()??!1,()=>(0,U.I5)(n,{method:"GET",headers:{Authorization:`Bearer ${t}`}}));if(s.ok){let e=(0,z.Al)(new Uint8Array(await s.arrayBuffer())),t=s.headers.get("content-type");return`data:${t};base64,${e}`}}return`https://avatars.githubusercontent.com/u/e?email=${encodeURIComponent(i)}&s=${o}`}async searchMyPullRequests(e,t,r,i){let o=(0,M.b1)();try{let n=function(t,r){return{pullRequest:(0,L.fromGitHubPullRequestDetailed)(t,e),reasons:r?[r]:[]}},a=`query searchPullRequests(
	$authored: String!
	$assigned: String!
...
}`,s=r?.search?.trim()??"";if(r?.user&&(s+=` user:${r.user}`),r?.repos!=null&&r.repos.length>0){let e="  repo:";s+=`${e}${r.repos.join(e)}`}let l="is:pr is:open archived:false",u=await this.graphql(e,t,a,{authored:`${s} ${l} author:@me`.trim(),assigned:`${s} ${l} assignee:@me`.trim(),reviewRequested:`${s} ${l} review-requested:@me`.trim(),mentioned:`${s} ${l} mentions:@me`.trim(),baseUrl:r?.baseUrl},o,i);if(void 0===u)return[];return ee([...u.assigned.nodes.map(e=>n(e,"assigned")),...u.reviewRequested.nodes.map(e=>n(e,"review-requested")),...u.mentioned.nodes.map(e=>n(e,"mentioned")),...u.authored.nodes.map(e=>n(e,"authored"))],e=>e.pullRequest.url)}catch(t){throw this.handleException(t,e,o)}}async searchMyIssues(e,t,r,i){let o=(0,M.b1)(),n=`query searchIssues(
				$authored: String!
				$assigned: String!
				$mentioned: String!
			) {
